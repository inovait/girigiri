...
export const DOCKER_UP_COMMAND = (composeFilePath: string) =>
  `docker compose -f "${composeFilePath}" up -d --build ${CONTAINER_SERVICE}`;

export const DOCKER_DOWN_COMMAND = (composeFilePath: string) =>   
  `docker compose -f "${composeFilePath}" rm -sfv ${CONTAINER_SERVICE}`;
