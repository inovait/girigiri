...
export const DOCKER_UP_COMMAND = (composeFilePath: string, envFilePath: string = '.env') =>
  `docker compose -f "${composeFilePath}" --env-file "${envFilePath}" up -d --build ${CONTAINER_SERVICE}`;

export const DOCKER_DOWN_COMMAND = (composeFilePath: string, envFilePath: string = '.env') =>
  `docker compose -f "${composeFilePath}" --env-file "${envFilePath}" rm -sfv ${CONTAINER_SERVICE}`;
